...
import { FiTrash2 } from "react-icons/fi"; // Assuming you're using Feather icons
...
const CartPage = () => {
  ...
  const updateQuantity = async (id, delta) => {
    try {
      ...
      window.location.reload(); // or ideally refetch user data via context
    } catch (err) {
      console.error(err);
    ...
    <div className="text-[#1e1e1e] font-sans mt-30 px-4 py-10 max-w-7xl mx-auto relative">
      ...
      <div className="grid lg:grid-cols-3 gap-10">
        ...
        <div className="sticky top-10 h-fit p-6 rounded-3xl bg-white/40 backdrop-blur-lg border border-white/30 shadow-lg">
          ...
          <div className="text-xl font-bold text-[#0f2c5c] mb-6">Grand Total: ₹ {getTotal()}</div>
          <button
            onClick={() =>
              navigate("/checkout", {
                state: {
